...
public class TodoServiceImpl extends AbstractModelServiceImpl<Todo> implements TodoService {
    ...
    /**
     * Finds a server with the given id and a username and returns an {@link Optional} {@link Todo}. second parameter is an optional boolean parameter
     * throwException that indicates if and when an exception should be thrown. If throwException is true and the server
     ...
     * whether the server belongs to a specific user or not, judged by the given username. if matchUsername is true and the server does not belong
